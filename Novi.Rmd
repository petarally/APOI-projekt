...
**Interpretacija:** Oba testa odbacuju H₀ o normalnosti za sve varijable (p < 0.05). Ovo je djelomično posljedica veličine uzorka (N = 1.044) koji amplificira statističku osjetljivost, ali i stvarnih odstupanja (desna asimetričnost `failures` i `absences`, bimodalni uzorak kod `g3`). Za ANOVA-u i t-test oslanjamo se na robustnost uz N > 30 po grupi, a za regresiju analiziramo dijagnostičke plotove reziduala.
...
- H₀: Niti jedan prediktor ne doprinosi značajno objašnjenju varijance završne ocjene (β₁ = β₂ = ... = βₖ = 0)
...
cat("F-statistika:", round(f_stat[1], 2),
    ...
    "| p =", format.pval(pf(f_stat[1], f_stat[2], f_stat[3], lower.tail = FALSE),
                         digits = 4), "\n")
```
...
**Interpretacija regresije:** Model objašnjava `r round(r2 * 100, 1)`% varijance završne ocjene G3 (R² = `r round(r2, 3)`, adj. R² = `r round(adj_r2, 3)`), što je iznimno visoka prediktivna snaga i odraz stvarnih, smislenih odnosa u podatcima. Dominantni prediktori su **G2** (β = `r round(coef(model_reg)["g2"], 3)`, p < 0.001) i **G1** (β = `r round(coef(model_reg)["g1"], 3)`, p < 0.001) - prethodne ocjene su daleko najjači prediktori završne ocjene. Varijabla **failures** (β = −0.237, p < 0.01) negativno predviđa završnu ocjenu — učenici s više prethodnih neuspjeha postižu niže završne ocjene, neovisno o tekućem uspjehu. Varijabla **absences** pokazuje pozitivan koeficijent (β = 0.034, p < 0.001), što je naizgled kontraintuitivno: ovaj supresijski efekt nastaje jer, kada se kontrolira za G1 i G2, izostanci postaju marker specifičnog ponašanja učenika koji prolaze usprkos izostancima te neovisno ne smanjuju G3 u modelu. Predmet (Portugalski vs. Matematika) statistički je značajan prediktor — kontrolirajući za sve ostale faktore, učenici portugalskog imaju prosječno više ocjene. Dijagnostički plotovi pokazuju relativno homogene rezidue bez jasnih uzoraka, što upućuje na adekvatnost pretpostavki linearnog modela. **H₀ se odbacuje** - višestruki prediktori statistički značajno objašnjavaju varijabilitet G3.
...
Višestruka linearna regresija s prediktorima G1, G2, studytime, failures, absences, goout, dalc i predmet objasnila je `r round(r2 * 100, 1)`% varijance završne ocjene (R² = `r round(r2, 3)`, adj. R² = `r round(adj_r2, 3)`). **H₀ se odbacuje** - višestruki prediktori statistički su i praktički značajni. Dominantni prediktori su G2 i G1 (prethodni uspjeh iz polugodišta), a `failures` negativno doprinosi predikciji. Predmet (Portugalski) statistički je značajan čak i kontrolirajući za prethodni uspjeh. Model ima stvarnu prediktivnu vrijednost. Dijagnostički plotovi ne ukazuju na ozbiljna kršenja pretpostavki linearnosti i homoskedastičnosti.
...
**3. Višestruka regresija s prethodnim ocjenama objašnjava gotovo svu varijancu G3 - H₀ se odbacuje.**
Model s G1, G2 i sekundarnim prediktorima objašnjava `r round(r2 * 100, 1)`% varijance G3 (R² = `r round(r2, 3)`). Prethodni uspjeh je daleko najjači prediktor budućeg uspjeha. Nakon kontrole za G1 i G2, broj prethodnih neuspjeha (`failures`) ostaje statistički značajan negativan prediktor, što implicira da akumulirano akademsko iskustvo nosi informaciju neovisno o tekućim ocjenama.
...
Sveukupno, nalazi su konzistentni s literaturom: prethodni akademski uspjeh je najsnažniji prediktor budućeg uspjeha, razlike između predmeta odražavaju stvarne strukturne razlike u težini ili ocjenjivanju, a socioekonomski kontekst (obrazovanje majke) igra statistički mjerljivu ulogu. Sve analize daju smislene i konzistentne rezultate, što potvrđuje da se radi o stvarnim, kvalitetnim obrazovnim podatcima.
